...
const LearningPathsSection = () => {
  ...
  const learningPaths = [
    {
      title: t('lp.beginnerTitle'),
      steps: [t('lp.beginnerStep1'), t('lp.beginnerStep2'), t('lp.beginnerStep3'), t('lp.beginnerStep4')],
      link: "/learning-paths",
    },
    {
      title: t('lp.investorTitle'),
      steps: [t('lp.investorStep1'), t('lp.investorStep2'), t('lp.investorStep3'), t('lp.investorStep4')],
      link: "/learning-paths",
    },
  ];
  ...
  return (
    <section className="py-36">
      <div className="container">
        <ScrollReveal>
          <p className="text-xs font-medium uppercase tracking-[0.3em] text-muted-foreground">{t('lpSection.label')}</p>
          ...
          {learningPaths.map((path, i) => (
            <ScrollReveal key={path.title} delay={i * 150}>
              <div className="flex flex-col h-full">
                ...
                >
                  {t('lpSection.startPath')} <ArrowRight className="h-4 w-4 transition-transform duration-300 group-hover:translate-x-1" />
                </Link>
              </div>
            </ScrollReveal>
          ))}
