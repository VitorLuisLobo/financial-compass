    ...
    <section className="py-36">
      <div className="container">
        <ScrollReveal className="text-center mb-16">
          <p className="text-xs font-medium uppercase tracking-[0.3em] text-muted-foreground">{t('libSection.label')}</p>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-base font-bold text-foreground group-hover:text-accent transition-colors duration-300" style={{ fontFamily: 'var(--font-display)' }}>
                      {t(catKeyMap[cat.slug])}
                    </h3>
                    <p className="mt-0.5 text-xs text-muted-foreground">
                      {libraryData.find((c) => c.slug === cat.slug)?.topics.length || 0} {t('libSection.topics')}
                    </p>
                  </div>
                </div>
