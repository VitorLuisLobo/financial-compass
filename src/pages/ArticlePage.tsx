  ...
  if (!article) {
    return (
      <div className="container py-20 text-center">
        ...
        <Link to="/blog" className="mt-4 inline-block text-accent hover:underline">← Back to blog</Link>
      </div>
    );
  }
      ...
      <article className="container py-16">
        ...
        <div className="prose mt-10 max-w-3xl">
          <p className="text-muted-foreground leading-relaxed">
            This is a placeholder for the full article content. In a production environment, this would contain the complete article text with rich formatting, images, and embedded content.
          </p>
        ...
        {article.relatedLibraryTopics.length > 0 && (
          <aside className="mt-12 rounded-lg border border-accent/20 bg-accent/5 p-6">
            ...
            <p className="mt-1 text-sm text-muted-foreground">Explore related topics in the Financial Library.</p>
        ...
        {relatedArticles.length > 0 && (
          <section className="mt-12">
            <h2 className="font-display text-xl text-foreground">More in {article.category}</h2>
            <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {relatedArticles.map((ra) => (
                <Link
                  key={ra.slug}
                  to={`/blog/${ra.slug}`}
                  className="group rounded-lg border border-border bg-card p-5 transition-all hover:border-accent/30 hover:shadow-md"
                >
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">{ra.readTime} read</p>
                </Link>
              ))}
            </div>
          </section>
